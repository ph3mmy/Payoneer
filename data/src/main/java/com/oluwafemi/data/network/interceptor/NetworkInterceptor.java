...
public class NetworkInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (!ConnectivityStatus.isConnected(context)) {
            ...
            throw new NoConnectionException(context.getString(R.string.error_not_connected));
        } else {
            try {
                ...
                switch (response.code()) {
                    case 401:
                        ...
                        throw new UnAuthorizedException(context.getString(R.string.error_unauthorized));
                    case 404:
                        networkEvent.publish(NetworkState.NOT_FOUND);
                        throw new NotFoundException(context.getString(R.string.error_resource_not_found));
                    case 500:
                        networkEvent.publish(NetworkState.SERVER_ERROR);
                        throw new ServerErrorException(context.getString(R.string.error_server_error));
                    case 503:
                        networkEvent.publish(NetworkState.NO_RESPONSE);
                        throw new NoResponseException(context.getString(R.string.error_no_response));
                }
                return response;
